...
def preprocess_graphs(graphs):
    ...
    for g in graphs:
        ...
        g.ndata['norm'] = norm
        #g.ndata['norm'].reshape((g.ndata['norm'].shape[0], 1))
...
def SpMM(DS, in_feats):
    ...
    for key in state_dict:
        new_key = key.replace("conv1.linear.", "conv1.").replace("conv2.linear.", "conv2.")
        new_state_dict[new_key] = state_dict[key]

    new_state_dict["conv1.weight"] = new_state_dict["conv1.weight"].t()
    new_state_dict["conv2.weight"] = new_state_dict["conv2.weight"].t()
    '''
    ...
    print("numpy_transform_1_B_time: ", numpy_transform_1_B_time)
    print("numpy_transform_2_B_time: ", numpy_transform_2_B_time)
    print("numpy_transform_1_W_time: ", numpy_transform_1_W_time)
    print("numpy_transform_2_W_time: ", numpy_transform_2_W_time)
    ...
    print("DAD_time1: ", DAD_time1)
    print("DAD_time1: ", DAD_time2)
    print("DAD_time1: ", DAD_time3)
    print("DAD_time1: ", DAD_time4)
    print("---------------------------------------------------------")
    ...
    print("---------------------------------------------------------")
    print("Infer time:", DAD_time1 + DAD_time2 + DAD_time3 + DAD_time4 + conv1 + conv2 + relu)
...
class SparseGCNConv(nn.Module):
    ...
    def forward(self, adj, features):
        if self.infeats < self.out_feats:
            ...
            start = time.time()
            sparse_feat = sparse_feat.detach().numpy()  # 确保转换为numpy数组
            end = time.time()
            if self.out_feats == 16:
                global numpy_transform_1_B_time
                numpy_transform_1_B_time += (end - start)
            else:
                global numpy_transform_2_B_time
                numpy_transform_2_B_time += (end - start)

            start = time.time()
            weight = self.weight.detach().numpy()  # 将权重转换为numpy数组
            end = time.time()
            if self.out_feats == 16:
                global numpy_transform_1_W_time
                numpy_transform_1_W_time += (end - start)
            else:
                global numpy_transform_2_W_time
                numpy_transform_2_W_time += (end - start)
            ...
            start = time.time()
            dense_feat = torch.sparse.mm(adj, dense_feat)
            end = time.time()
            if self.out_feats == 16:
                # global KeyMM_1_time
                KeyMM_1_time += (end - start)
            ...
            start = time.time()
            dense_feat = dense_feat.detach().numpy()  # 确保转换为numpy数组
            end = time.time()
            if self.out_feats == 16:
                # global numpy_transform_1_B_time
                numpy_transform_1_B_time += (end - start)
        ...
        if self.bias is not None:
            ...
            if self.out_feats == 16:
                global numpy_transform_1_bias_time
                numpy_transform_1_bias_time += (end - start)
            else:
                global numpy_transform_2_bias_time
                numpy_transform_2_bias_time += (end - start)
            start = time.time()
            dense_feat += bias
            end = time.time()
            if self.out_feats == 16:
