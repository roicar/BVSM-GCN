...
def prepare_params(g):
    ...
    params["norm"] = np.power(degs, -0.5,where=degs>0).astype("float32")
...
def GraphConv(layer_name, input_dim, output_dim, adj, input, norm=None, bias=True, activation=None):
    ...
    if output_dim == 16:
        global DAD_1_1_time
        DAD_1_1_time += (end - start)
    else:
        global DAD_2_1_time
        DAD_2_1_time += (end - start)
    ...
    if norm is not None:
        ...
        if output_dim == 16:
            global DAD_1_2_time
            DAD_1_2_time += (end - start)
        else:
            global DAD_2_2_time
            DAD_2_2_time += (end - start)
    if bias is True:
        start = time.time()
...
for dgl_g in dataset:
    ...
    norm = relay.op.transform.expand_dims(norm, axis=0, num_newaxis=1)
