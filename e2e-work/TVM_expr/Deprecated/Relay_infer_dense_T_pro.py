...
def GraphConv(layer_name, input_dim, output_dim, adj, input, norm=None, bias=True, activation=None):
    ...
    weight_t = relay.op.transform.expand_dims(weight_t, axis=0, num_newaxis=1)
    end = time.time()
    global expendims_time
    expendims_time += (end - start)

    start = time.time()
    dense_1 = relay.nn.batch_matmul(weight_t,input)  # 修改此行，使用稠密矩阵乘法
    end = time.time()
    if output_dim == 16:
        global HW_1_time
        HW_1_time += (end - start)
    ...
    start = time.time()
    dense_2 = relay.nn.batch_matmul(dense_1, adj)
    end = time.time()
    ...
    output = relay.transpose(dense_2)
    if norm is not None:
        start = time.time()
