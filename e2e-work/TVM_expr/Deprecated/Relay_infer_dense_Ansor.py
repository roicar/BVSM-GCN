...
graph_sizes = [(i, g.number_of_nodes()) for i, g in enumerate(dataset)]
...
def GraphConv(layer_name, input_dim, output_dim, adj, input, norm=None, bias=True, activation=None):
    ...
    start = time.time()
    #调优部分替换relay.nn.dense为relay.op.nn.dense
    output = relay.op.nn.dense(dense, adj)
    end = time.time()
    global KeyMM_time
    ...
    end1 = time.time()
    global Graphconv_time
    Graphconv_time += (end1 - start1)
    return output_t
...
# 这需要你提供具体的输入尺寸和数据类型
# 示例调优代码（假设任务已经定义）
from tvm.auto_scheduler import SearchTask,TuningOptions, auto_scheduler
...
for dgl_g in dataset:
    ...
    target = "llvm -mcpu=core-avx2"  # Currently only support `llvm` as target
