...
def GraphConv(layer_name, input_dim, output_dim, adj, input, bias=True, activation=None):
    ...
    # start = time.time()
    output = relay.nn.batch_matmul(adj,input,transpose_b=False)
    # end = time.time()
    # if output_dim == 16:
    #     global AH_1_time
    #     AH_1_time += (end - start)
...
for dgl_g in dataset:
    ...
    for i in range(1,4):
        params["conv%d.weight" % (i + 1)] = model_params["conv2.weight"]
        params["conv%d.bias" % (i + 1)] = model_params["conv2.bias"]
...
print("MinexeTime:", MinexeTime)
print("MaxexeTime:", MaxexeTime)
