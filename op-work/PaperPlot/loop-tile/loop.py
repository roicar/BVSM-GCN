...
def Data_power2_random_TEST():
    ...
    y_1 = [0.26,0.505,1.092,1.234,2.006]
    y_2 = [0.21,0.462,0.971,1.136,1.669]
    plt.tick_params(labelsize=14)
    plt.grid(linestyle='-.', axis='y')

    x = np.arange(len(y_Ansor))
    
    a5 = plt.bar(x-0.1, y_1, 0.2, color='yellow', label='(8,8,8)', align='center')
    a6 = plt.bar(x+0.1, y_2, 0.2, color='red', label='(16,16,16)', align='center')
    
    plt.legend(labels=['(8,8,8)','(16,16,16)'], loc='upper left', fontsize=14)
    
    plt.xticks(x, ['SSRSRS','SRSRS','RSRS','SRS','RS'],size=10,rotation=90)
    plt.savefig('不同循环层数的性能影响.pdf',format='pdf')
