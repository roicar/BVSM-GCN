...
def Optimization_time_nor():
    ...
    fig, ax = plt.subplots()
    plt.ylabel('Speed-up(x)', fontsize=18)
    ...
    a1 = plt.bar(x-0.1, MKL, 0.2, color='white', label='Intel MKL Batched', align='center', edgecolor='k', hatch='/')
    a2 = plt.bar(x+0.1, BVSM, 0.2, color='white', label='BVSM-G', align='center', edgecolor='k', hatch='')
   
    #plt.axhline(1.0, color='red',lw=1,linestyle='--')
    plt.xticks(x, ['D1','D2','D3', 'D4','D5'],size=18,rotation=0)
    plt.ylim(0.0, )
    plt.legend(loc='upper left', fontsize=18)
    plt.savefig('CPU_intel3_SpeedUp.pdf',format='pdf', dpi=1000)
    #plt.show()
