...
def Optimization_time_nor():
    ...
    '''
    a4 = plt.bar(x-0.3, Ansor, 0.2, color='blue', label='Ansor', align='center')
    a5 = plt.bar(x-0.1, Atch, 0.2, color='yellow', label='Atch', align='center')
    a6 = plt.bar(x+0.1, Atch_padToMax, 0.2, color='red', label='Atch_padToMax', align='center')
    a7 = plt.bar(x+0.3, Atch_padGroup, 0.2, color='pink', label='Atch_padGroup', align='center')
    '''
    # 设置标签
    for i in a + b:
        h = i.get_height()
