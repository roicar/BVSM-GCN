...
def naive_GEMM(DS):
    ...
    '''
    shape_A = np.shape(matrices_A)
    print("matrices_A的第一维度:", shape_A[0])
    for i in range(0,len(matrices_A)):
        print("shape_A的第%d个矩阵的M维度:"%(i),len(matrices_A[i][0]))
        print("shape_A的第%d个矩阵的K维度:"%(i),len(matrices_A[i][1]))
    '''
...
def mytuner(n_trial, M, K, N, i, early_stopping, tuning_tmp_time):
    ...
    if len(file_list) > 0:
        ...
        print("%s 的第 %d 个维度GEMM 的调优时间（已重复）为：%f.3 s" % (DS, i, tuning_tmp_time))
    else:
